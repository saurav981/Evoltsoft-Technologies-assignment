...
const isFormValid = computed(() => {
  return (
    newCharger.value.name.trim() !== '' &&
    newCharger.value.location.lat !== null &&
    newCharger.value.location.lat !== '' &&
    newCharger.value.location.lng !== null &&
    newCharger.value.location.lng !== '' &&
    newCharger.value.powerOutput !== null &&
    newCharger.value.powerOutput !== '' &&
    newCharger.value.powerOutput > 0
  )
})

const validateField = (field) => {
  if (field === 'name') {
    errors.value.name = newCharger.value.name.trim() === ''
  } else if (field === 'lat') {
    errors.value.lat =
      newCharger.value.location.lat === null || newCharger.value.location.lat === ''
  } else if (field === 'lng') {
    errors.value.lng =
      newCharger.value.location.lng === null || newCharger.value.location.lng === ''
...
const handleSubmit = () => {
  ...
  errors.value = { name: false, lat: false, lng: false, powerOutput: false }
}
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="group">
              ...
              <input
                v-model.number="newCharger.location.lat"
                type="number"
                step="0.000001"
                class="w-full px-4 py-3 border-0 rounded-xl focus:outline-none focus:ring-2 transition-all duration-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 text-gray-900 dark:text-white"
                :class="
            ...
            <div class="group">
              ...
              <input
                v-model.number="newCharger.location.lng"
                type="number"
                step="0.000001"
                class="w-full px-4 py-3 border-0 rounded-xl focus:outline-none focus:ring-2 transition-all duration-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 text-gray-900 dark:text-white"
                :class="
          ...
          <div class="group">
            ...
            <div class="flex gap-3">
              <label class="flex items-center cursor-pointer group/radio">
                <input type="radio" v-model="newCharger.status" value="Active" class="sr-only" />
                <div
                  ...
                  :class="newCharger.status === 'Active' ? 'border-green-500 bg-green-500' : ''"
                >
                  <div
                    v-if="newCharger.status === 'Active'"
              ...
              <label class="flex items-center cursor-pointer group/radio">
                <input type="radio" v-model="newCharger.status" value="Inactive" class="sr-only" />
                <div
                  ...
                  :class="newCharger.status === 'Inactive' ? 'border-red-500 bg-red-500' : ''"
                >
                  <div
                    v-if="newCharger.status === 'Inactive'"
                    class="w-2 h-2 bg-white rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                  ></div>
                </div>
                <span
